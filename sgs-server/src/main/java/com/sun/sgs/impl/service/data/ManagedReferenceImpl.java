...
final class ManagedReferenceImpl<T> implements ManagedReference<T>,
	...
	T getForUpdate(boolean checkContext) {
		...
		try {
			...
			switch (state) {
			case EMPTY:
				unmodifiedBytes = context.store.getObject(context.txn, oid,
						!context.optimisticWriteLocks());
				object = deserialize(unmodifiedBytes);
				if (!context.detectModifications)
					unmodifiedBytes = null;
				context.refs.registerObject(this);
				context.store.setObjectDescription(context.txn, oid, object);
	...
	byte[] flush() {
		...
		case MODIFIED:
			...
			if (unmodifiedBytes != null
					&& Arrays.equals(result, unmodifiedBytes)) {
				debugDetectLogger.log(Level.SEVERE,
						"Unmodified was marked for update: {0}",
	...
	private transient List<StackTraceElement[]> stackTraces = null;

	private List<StackTraceElement[]> getStackTraces() {
		if (stackTraces == null)
			stackTraces = new ArrayList<StackTraceElement[]>();
		return stackTraces;
	}
	...
	private void log() {
		...
		if (State.EMPTY.equals(state))
			return;
		getStackTraces().add(Thread.currentThread().getStackTrace());
	}

	private void printObjectInfo() {
		String name = this.getClass().getName();
		for (StackTraceElement[] t : getStackTraces()) {
			int i = 1;
			while (i < t.length && t[i].toString().startsWith(name))
				i++;
			while (i < t.length) {
				StackTraceElement e = t[i];
